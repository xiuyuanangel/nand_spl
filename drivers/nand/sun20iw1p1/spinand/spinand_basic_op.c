...
//			txnum = 4;
        ...
        sdata[0] = 0x0b;
//        printf("%s %d -- %x %x %x %x %x %x\n",__FILE__,__LINE__,rxnum,sdata[0],sdata[1],sdata[2],sdata[3],sdata[4]);
		       ...
		       __u32 data_size, __u32 spare_only_flag)
{
	...
	if (spare_only_flag)
		ret = spi_nand_read_x1(0, addr, 0, 1, NULL, readop->oobbuf, 0);
/*		ret = spi_nand_read_x1(0, addr, 0, 16, NULL, readop->oobbuf, 0); ** 2022.3.31 wei */
	else
